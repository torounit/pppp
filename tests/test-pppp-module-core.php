...
Class Test_PPPP_Module_Core extends WP_UnitTestCase {
	...
	public function test_post_type() {
		...
		update_option( 'posts_per_page_of_cpt_' . $post_type, 2 );
		...
		add_action( 'wp', function () use( $self ) {
			...
			$wp_query;
			$self->assertCount( 2, $wp_query->posts );
		} );
	...
	public function test_taxonomy() {
		...
		update_option( 'posts_per_page_of_tax_' . $taxonomy, 3 );
		...
		add_action( 'wp', function () use( $self ) {
			...
			$wp_query;
			//var_dump($wp_query);
			$self->assertCount( 3, $wp_query->posts );
		} );
	...
	public function test_category() {
		update_option( 'posts_per_page_of_tax_category', 4 );
		...
		add_action( 'wp', function () use( $self ) {
			global /** @var WP_Query $wp_query */
			$wp_query;
			$self->assertCount( 4, $wp_query->posts );
		} );
